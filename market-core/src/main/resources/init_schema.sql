...
CREATE TABLE IF NOT EXISTS dispute_channel (id SERIAL, name CHARACTER VARYING(255), description CHARACTER VARYING(255), unread INT, PRIMARY KEY (id));
...
DECLARE
  payload TEXT;
BEGIN
  payload := json_build_object(''id'', NEW.id, ''name'', NEW.name, ''description'', NEW.description, ''unread'', NEW.unread);
